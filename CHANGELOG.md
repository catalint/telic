...
- **Cross-realm dispatch (core, mediate, wire, new transport):** `dispatch` can now reach a handler in another realm — iframe, worker, tab — with the caller owning the channel and telic contributing only correlation. New `@telic/core/transports/remote-dispatch` leaf (`createRemoteDispatcher` / `receiveRemoteDispatches`, 557 B brotli); `beginRemote`/`executeRemote` on mediate (module-level and `createMediator`); a distinct S19.4 request envelope on the wire (never confusable with the mark envelope); and the now-normative S10.9 ingest completion invariant — "ingest may complete local lifecycles, never start them" — which settles the caller's live attempt from the returning marks at zero new core cost. Adoption is echo-safe by contract (S15.10 three-way resolution: observing a caller's `begun` never blocks executing its request). Honest limits are spec'd, not implied: no delivery guarantee (silence → abandon on the caller's own deadline), single settlement but not single execution across realms (the AttemptId-as-Idempotency-Key is the server-side mitigation). Additive. (D34, SPEC S10.9/S15.9/S15.10/S19.4/S28)
- **Examples:** new `packages/core/docs/examples/` — worked end-to-end use cases: the cross-realm support widget (flagship), worker offload, and the same-realm checkout `flow()` saga, indexed and linked from the README documentation table.
- **react (0.1.3):** widened the `@telic/core` peer range to `^0.3.0 || ^0.4.0 || ^0.5.0` so the adapter installs against the 0.5.0 core. No code change — the React adapter never touched the D31/D32/D34 surface.
